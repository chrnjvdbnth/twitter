	...
	if(isset($_POST['screenName'])){
		if(!empty($_POST['screenName'])){
			...
			if(strlen($screenName) > 20){
				...
				$getFromU->update('users', $user_id, array('bio' => $profileBio));
				$getFromU->update('users', $user_id, array('country' => $country));
				$getFromU->update('users', $user_id, array('website' => $website));
